...
def _upload_to_s3_if_configured(job_id: str, file_path: Path, result_key: str, result: dict, errors: list) -> None:
    """If BUCKET_ENDPOINT_URL is set, upload file to S3 and set result[result_key] to URL; on error append to errors."""
    ...
    try:
        s3_url = rp_upload.upload_image(job_id, path_str)
        result[result_key] = s3_url
        print(f"[MegaSaM] Uploaded {file_path.name} to S3: {s3_url}", flush=True)
    except Exception as e:
        errors.append(f"Error uploading {file_path.name} to S3: {e}")
...
def run_pipeline(
    ...
    # 5) Export npz to COLMAP format (three txt files only; no image frame folder)
    ...
    # Only output the 3 COLMAP txt files; images.txt first (most important).
...
def handler(event):
    ...
    try:
        ...
        try:
            ...
            # S3 upload: only the 3 COLMAP txt files (images.txt first).
            job_id = event.get("id") or "unknown"
            upload_errors = []
            colmap_dir = INSTALL_DIR / "outputs" / f"colmap_{SCENE_NAME}"
            if os.environ.get("BUCKET_ENDPOINT_URL") and job_id:
                _upload_to_s3_if_configured(job_id, colmap_dir / "images.txt", "colmap_images_txt_url", out, upload_errors)
                _upload_to_s3_if_configured(job_id, colmap_dir / "cameras.txt", "colmap_cameras_txt_url", out, upload_errors)
                _upload_to_s3_if_configured(job_id, colmap_dir / "points3D.txt", "colmap_points3d_txt_url", out, upload_errors)
                if upload_errors:
                    out["upload_errors"] = upload_errors

            # Return only the 3 COLMAP outputs (images first), timings, and optional upload_errors.
            print(
